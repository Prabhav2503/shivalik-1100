...
export default function AdminLibraryPage() {
  ...
  const [selectedBooking, setSelectedBooking] = useState<LibraryBooking | null>(
    null,
  );
  ...
  const fetchBookings = async () => {
    try {
      setLoading(true);
      const response = await axios.get<LibraryBooking[]>(
        "/api/admin/libraryBookings",
      );
  ...
  const handleUpdateStatus = async (id: string, newStatus: string) => {
    try {
      await axios.put(`/api/admin/library-bookings?id=${id}`, {
        status: newStatus,
      });
                    ...
                    {(bookingsByDate[selectedDate] || []).map((booking) => (
                      ...
                      >
                        <div className="flex justify-between mb-2">
                          ...
                          <div
                            className={`text-sm px-2 py-1 rounded-full ${getStatusBadgeClass(booking)}`}
                          >
                            {isOverdue(
                              booking.date,
                              booking.duration,
                        ...
                        <div className="text-sm text-gray-400 mt-2">
                          Return by:{" "}
                          {formatDate(
                            calculateEndDate(booking.date, booking.duration),
                          )}
                        </div>
                      </div>
                    ))}
            ...
            <table className="min-w-full">
              ...
              <tbody>
                {filteredBookings
                  ...
                  .map((booking) => {
                    const bookingOverdue = isOverdue(
                      booking.date,
                      booking.duration,
                      booking.status,
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <span
                            className={`px-2 py-1 rounded-full text-xs ${getStatusBadgeClass(booking)}`}
                          >
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="flex items-center space-x-2 mt-1">
                  <span
                    className={`px-2 py-1 rounded-full text-xs ${getStatusBadgeClass(selectedBooking)}`}
                  >
                    {isOverdue(
                      selectedBooking.date,
                      selectedBooking.duration,
            ...
            <div className="mt-6 flex justify-between">
              <div>
                {selectedBooking.status === "pending" && (
                  <button
                    onClick={() =>
                      handleUpdateStatus(selectedBooking._id, "lended")
                    }
                ...
                {(selectedBooking.status === "lended" ||
                  ...
                  <button
                    onClick={() =>
                      handleUpdateStatus(selectedBooking._id, "returned")
                    }
